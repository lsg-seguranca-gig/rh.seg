...
//  CONFIGURAÇÃO
// ─────────────────────────────────────────────────────────────────
const ADMIN_PASSWORD = "SegurancaLSG2026";   // ← senha atualizada
const PROXY = "/api/gas";
...
export default function App() {
  ...
  // Estado do Inventário: cópia editável das quantidades
  const [invDraft, setInvDraft]           = useState({});   // { id: novaQtd }
  ...
  const loadSheet = useCallback(async () => {
    ...
    try {
      ...
      setEpis(loaded);
      // Inicializa rascunho do inventário com quantidades atuais
      const draft = {};
      loaded.forEach(e => { draft[e.id] = e.quantidade; });
      setInvDraft(draft);
      setInvDirty(false);
      ...
      setEpis(DEMO_EPIS);
      const draft = {};
      DEMO_EPIS.forEach(e => { draft[e.id] = e.quantidade; });
      setInvDraft(draft);
  ...
  const invFiltered = epis.filter(e =>
    e.nome.toLowerCase().includes(invSearch.toLowerCase()) ||
    e.ca.toLowerCase().includes(invSearch.toLowerCase()) ||
    e.local.toLowerCase().includes(invSearch.toLowerCase())
  );
              ...
              {tab === "estoque" && (
                <div>
                  ...
                  <div style={s.readonlyBanner}>
                    🔒 Visualização somente leitura — para alterar quantidades use a aba <strong>Inventário</strong>
                  </div>
                ...
                <div>
                  <div style={s.invHeader}>
                    <div>
                      ...
                      <p style={s.invSubtitle}>Atualize as quantidades e clique em <strong>Salvar Inventário</strong> para gravar na planilha.</p>
                    </div>
                    <div style={s.invActions}>
                      <button style={{ ...s.saveBtn, opacity: invDirty ? 1 : 0.5, cursor: invDirty ? "pointer" : "default" }}
                  ...
                  <div style={s.tableWrap}>
                    <table style={s.table}>
                      ...
                      <tbody>
                        {invFiltered.length === 0 && <tr><td colSpan={8} style={s.emptyCell}>Nenhum EPI encontrado.</td></tr>}
                        {invFiltered.map(epi => {
                          ...
                          return (
                            <tr key={epi.id} style={{ ...s.tr, ...(baixo ? s.trAlert : {}) }}>
                              ...
                              {/* Quantidade atual (referência) */}
                              <td style={{ ...s.td, textAlign: "center", color: C.textSub }}>{epi.quantidade}</td>
                              {/* Campo editável */}
                              <td style={s.td}>
                                <div style={s.qtyRow}>
                                  <button style={s.qtyBtn} onClick={() => handleInvChange(epi.id, novaQtd - 1)}>−</button>
                                  <input
                                    style={s.qtyInput}
                                    type="number" min="0"
                                    value={novaQtd}
                                    onChange={e => handleInvChange(epi.id, e.target.value)}
                                  />
                                  <button style={s.qtyBtn} onClick={() => handleInvChange(epi.id, novaQtd + 1)}>+</button>
                                </div>
                              </td>
...
function LogoImg({ style }) {
  ...
  return (
    <img src={names[idx]} alt="LSG Sky Chefs" style={style}
...
function makeStyles(C) {
  return {
    ...
    tabs:         { display: "flex", gap: 4, borderBottom: `2px solid ${C.tabBorder}`, marginBottom: 24, flexWrap: "wrap" },
    tab:          { background: "none", border: "none", padding: "10px 18px", cursor: "pointer", fontWeight: 600, fontSize: 14, color: C.textSub, borderBottom: "2px solid transparent", marginBottom: -2, transition: "all 0.15s" },
    ...
    // Banner somente leitura
    readonlyBanner: { background: C.okBg, border: `1px solid ${C.border}`, color: C.textSub, borderRadius: 8, padding: "10px 16px", marginBottom: 14, fontSize: 13 },
    // Inventário
    invHeader:    { display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16, gap: 12, flexWrap: "wrap" },
    invTitle:     { margin: "0 0 4px", fontSize: 20, fontWeight: 800, color: C.primary },
    invSubtitle:  { margin: 0, fontSize: 13, color: C.textSub },
    invActions:   { display: "flex", gap: 10, alignItems: "center", flexShrink: 0 },
    invDirtyBanner: { background: "#fffbeb", border: "1px solid #f6e05e", color: "#744210", borderRadius: 8, padding: "10px 16px", marginBottom: 14, fontSize: 13, fontWeight: 600 },
    qtyInput:     { width: 64, padding: "5px 8px", border: `1.5px solid ${C.border}`, borderRadius: 6, fontSize: 14, textAlign: "center", background: C.inputBg, color: C.textMain, outline: "none" },
    // Tabela
    ...
    qtyBtn:       { width: 26, height: 26, border: `1.5px solid ${C.border}`, borderRadius: 6, background: C.inputBg, cursor: "pointer", fontWeight: 700, fontSize: 15, lineHeight: 1, color: C.textMain },
    ...
    alertCard:    { background: C.surface, border: `1.5px solid ${C.alertBorder}`, borderRadius: 12, padding: 20 },
    ...
    loginWrap:    { minHeight: "calc(100vh - 88px)", display: "flex", alignItems: "center", justifyContent: "center" },
