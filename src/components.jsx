...
export function DashboardTab({ epis, dashboard, s, C, onGoToAlertas }) {
  ...
  // Cores fixas e distintas por local — com fallback para qualquer outro valor
  const PALETA = [C.primary, C.ok, "#dd6b20", "#8854d0", "#319795"];
  const pieData = Object.entries(porLocal).map(([local, val], i) => ({
    label: local, value: val,
    color: local === "Almoxarifado" ? C.ok
         : local === "Segurança do Trabalho" ? C.primary
